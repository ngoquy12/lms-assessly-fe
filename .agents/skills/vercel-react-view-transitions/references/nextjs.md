...
function DetailButton({ href }: { href: string }) {
    ...
    return <button onClick={() => router.push(href, { transitionTypes: ["nav-forward"] })}>Open</button>;
}
```
...
```tsx
"use client";
import { ViewTransition, useOptimistic, useTransition } from "react";
...
```tsx
<ViewTransition enter={{ "nav-forward": "slide-from-right", default: "none" }} exit={{ "nav-forward": "slide-to-left", default: "none" }} default="none">
    <div>
        <Suspense
            fallback={
