...
export function synthesizeImpactFromSignal(rec, signals = {}) {
    ...
    const sig = [rec?.o11ySignal, rec?.evidence?.o11ySignal, rec?.why, rec?.what].filter((v) => typeof v === "string" && v.trim()).join("\n") || null;
    ...
    const buildSharePct = parseSigNumber(m, /build_minutes_share=([\d.]+)%/i) ?? parseSigNumber(m, /build(?: CPU)? minutes share:?\s*([\d.]+)%/i);
