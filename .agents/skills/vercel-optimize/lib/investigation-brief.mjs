...
export function inferPlaybook(signals) {
    ...
    const usageHas = (re, minBilled = 0) => services.some((s) => re.test(String(s?.name ?? "")) && Number(s?.billedCost ?? s?.cost ?? 0) > minBilled);
    ...
    const aiDep = has("@vercel/sandbox") || has("@vercel/ai-gateway") || has("ai") || has("openai") || has("@anthropic-ai/sdk") || hasPrefix("@ai-sdk/");
    const aiUsage = usageHas(/^AI Gateway$/i) || usageHas(/^Sandbox/i);
    if (aiDep || aiUsage) {
        return "ai-application";
    }
    if (has("stripe") || has("@stripe/stripe-js") || has("react-stripe-js") || anyRouteMatches(/^\/(cart|checkout|products?)\b/i)) {
        return "ecommerce";
    }
    if (has("next-auth") || has("@clerk/nextjs") || has("@workos-inc/authkit-nextjs") || anyRouteMatches(/^\/(admin|dashboard|settings|account|billing)\b/i)) {
        return "saas";
    }
...
export function resolveFiles(candidate, signals) {
    ...
    if (matched.length === 0) {
        ...
        const scored = nonLayoutRoutes.map((r) => ({ r, score: routePathMatchScore(r.routePath, route) })).filter((x) => x.score > 0);
    ...
    const workspaceImports = matched.flatMap((r) => (Array.isArray(r.workspaceImports) ? r.workspaceImports : [])).filter(Boolean);
...
function briefRoots(signals) {
    ...
    const appRoot = typeof codebase.rootDir === "string" && codebase.rootDir.length > 0 ? normalize(codebase.rootDir) : null;
    const repoRoot = typeof codebase.monorepoRoot === "string" && codebase.monorepoRoot.length > 0 ? normalize(codebase.monorepoRoot) : appRoot;
...
function capBriefFiles(nonLayoutCandidates, layoutCandidates, routes) {
    ...
    return [...uniq(nonLayout).slice(0, NON_LAYOUT_FILE_CAP), ...uniq(layouts).slice(0, LAYOUT_FILE_CAP)];
}
...
function closestAncestorLayoutFiles(route, routes) {
    ...
    return routes
        ...
        .sort((a, b) => routeDepth(b.routePath) - routeDepth(a.routePath) || a.file.localeCompare(b.file))
        .map((r) => r.file);
}
...
export async function citationSubset(candidateKind, framework, version) {
    ...
    const versionOk = (entry) =>
        entry.applicableFrameworks.includes("*") || entry.applicableFrameworks.some((p) => matchesFrameworkVersion(p, framework, version));
    const kindOk = (entry) => {
...
export function buildBrief({
    ...
    if (Array.isArray(files) && files.length > 0) {
        ...
        for (const f of files) {
            const tag = layoutFiles.has(f) || isLayoutPath(f) ? "(layout)" : routeFiles.has(f) ? "(route)" : "(workspace import)";
