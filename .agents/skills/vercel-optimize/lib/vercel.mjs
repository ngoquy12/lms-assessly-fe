...
function resolveVercelPackageEntry(dir, exists) {
    const packageRoots = [win32.join(dir, "node_modules", "vercel"), win32.join(win32.dirname(dir), "vercel")];
...
export async function checkCliVersion() {
    ...
    for (let i = 0; i < 3; i++) {
        ...
        if (v[i] < MIN_CLI_VERSION[i]) {
            throw new Error(`VERCEL_CLI_TOO_OLD: have ${v.join(".")}, need >= ${MIN_CLI_VERSION.join(".")}. Upgrade with \`npm i -g vercel@latest\`.`);
        }
    }
    return v;
}
...
export async function resolveProjectId(explicit, cwd = process.cwd()) {
    if (explicit) {
        const linked = process.env.VERCEL_ORG_ID ? null : await readLinkedOwnerForProjectId(explicit, cwd);
        return {
            projectId: explicit,
    ...
    if (process.env.VERCEL_PROJECT_ID) {
        const linked = process.env.VERCEL_ORG_ID ? null : await readLinkedOwnerForProjectId(process.env.VERCEL_PROJECT_ID, cwd);
        return {
...
async function readLinkedOwnerForProjectId(projectId, cwd = process.cwd()) {
    try {
        const raw = await readFile(join(cwd, ".vercel", "repo.json"), "utf-8");
        const parsed = JSON.parse(raw);
        const matches = (Array.isArray(parsed?.projects) ? parsed.projects : []).filter((p) => p?.id && String(p.id) === String(projectId));
        if (matches.length > 1) {
            throw new Error(
    ...
    if (stdout && stdout.trim().startsWith("{")) {
        try {
            ...
            if (data && typeof data === "object" && data.error) {
                ...
                return isDailyQuotaExceeded(failure) ? { ...failure, code: "DAILY_QUOTA_EXCEEDED", originalCode: failure.code } : failure;
            }
...
function categorizeError(exitCode, stderr) {
    ...
    if (lc.includes("permission") || lc.includes("not authorized") || lc.includes("403")) return "FORBIDDEN";
...
export function classifyObservabilityPlusConfiguration(result, { projectId } = {}) {
    ...
    if (result?.ok) {
        ...
        const disabled = projectId ? disabledProjects.find((p) => String(p?.id ?? "") === String(projectId)) : null;
        if (disabled) {
            return {
                ok: true,
                source,
...
export async function queryMetric(metricId, opts = {}) {
    ...
    return await throttle.run(() => retryOnRateLimit(() => runVercelJson(scopedArgs(args, opts.scope)), { onRetry }));
}
...
export async function getProjectConfig(projectId, orgId) {
    const qs = orgId && !String(orgId).startsWith("usr_") ? `?teamId=${encodeURIComponent(orgId)}` : "";
...
function extractPlanOption(accountPlan) {
    ...
    return {
        plan,
        reason: accountPlan.reason ?? (accountPlan.source ? `${accountPlan.source}=${plan}` : `billing.plan=${plan}`),
...
export async function detectStack(cwd = process.cwd()) {
    ...
    const orm = deps.prisma || deps["@prisma/client"] ? "prisma" : deps["drizzle-orm"] ? "drizzle" : deps.kysely ? "kysely" : "none";
    const vercelFlagsPackages = ["@vercel/flags", "@vercel/flags/next", "@vercel/flags/sveltekit", "@vercel/flags/nuxt"].filter((name) => deps[name]);
    const workflowPackages = Object.keys(deps)
        .filter((name) => name === "workflow" || name.startsWith("@workflow/"))
        .sort();

    const isMonorepo = !!pkg.workspaces || (await pathExists(join(cwd, "pnpm-workspace.yaml"))) || (await pathExists(join(cwd, "lerna.json")));
