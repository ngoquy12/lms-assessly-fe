...
export function buildResolver(packages) {
    ...
    return function resolveSpecifier(specifier) {
        ...
        const candidates = [...byName.keys()].filter((name) => specifier === name || specifier.startsWith(name + "/")).sort((a, b) => b.length - a.length);
...
async function selectRelevantForwards(fromFile, refs, requestedNames, resolver) {
    ...
    for (const [index, ref] of refs.entries()) {
        ...
        if (!next) continue;
        let score = requestedNames && requestedNames.size > 0 ? forwardRelevanceScore(ref, requestedNames, refs.length) : 1;
    ...
    const ranked = resolved.filter((x) => x.score > 0).sort((a, b) => b.score - a.score || a.index - b.index);
