...
function observationText(observation) {
    return [observation?.summary, observation?.evidence, observation?.suggestedAction].filter(Boolean).join(" ");
}
...
function hasUnsupportedWafBotCategoryClaim(observation) {
    ...
    return /\bbot_category\s*=/i.test(text) || /\btarget(?:ing)?\s+(?:browser_impersonation|automated_browser|ecommerce|monitor)\b/i.test(text);
}
...
function contradictsNoChangeReason(observation, abstentions) {
    ...
    const relevantReasons = abstentions.filter((a) => candidateTarget(a?.candidateRef) === target).map((a) => String(a?.reason ?? "").toLowerCase());
