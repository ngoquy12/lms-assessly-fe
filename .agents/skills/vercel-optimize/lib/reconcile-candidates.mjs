...
const SCANNER_ONLY_KINDS = new Set(["cache_header_gap", "image_optimization", "rendering_candidate"]);
...
export function reconcileInvestigation(investigation, { gate = null } = {}) {
    ...
    const priorPreResolved = Array.isArray(investigation.preResolvedRecords) ? investigation.preResolvedRecords : [];
...
function routeErrorsMetricMismatchDecision(candidate, ctx) {
    ...
    const rows = [...arrayAt(candidate, ["evidence", "deepDive", "errorStatusPattern"]), ...arrayAt(candidate, ["evidence", "deepDive", "errorsByDeployment"])];
    if (rows.length === 0) return null;
    let confirmed5xx = 0;
    for (const row of rows) {
...
export function candidateRefFor(candidate, files = candidate?.files) {
    ...
    const target = candidate.route ?? candidate.hostname ?? (Array.isArray(files) && files.length > 0 ? `<account>#${files[0]}` : "<account>");
