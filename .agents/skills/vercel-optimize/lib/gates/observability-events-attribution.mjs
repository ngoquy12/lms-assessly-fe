...
export function gate(signals) {
    ...
    const eventsBilled = services.filter((s) => EVENTS_RE.test(String(s?.name ?? ""))).reduce((acc, s) => acc + Number(s.billedCost ?? s.cost ?? 0), 0);
    ...
    return [
        {
            ...
            reason: critical ? "observability events exceed 30% of total billed cost" : "observability events exceed 20% of total billed cost",
