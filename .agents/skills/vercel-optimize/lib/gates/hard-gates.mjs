...
export const VERCEL_FLAGS_PACKAGES = ["@vercel/flags", "@vercel/flags/next", "@vercel/flags/sveltekit", "@vercel/flags/nuxt"];
...
export function isWorkflowRuntimeEndpointCandidate(candidate) {
    ...
    return WORKFLOW_ENDPOINT_PREFIXES.some((prefix) => route === prefix || route.startsWith(`${prefix}/`));
}
