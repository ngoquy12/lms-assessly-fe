...
export function gate(signals) {
    ...
    const edgeService = (signals.usage?.services ?? []).find((s) => /edge.request/i.test(s.name ?? ""));
    ...
    const ruleNote = challengeRule?.active ? `firewall bot_filter rule active (action=${challengeRule.action ?? "?"})` : "no firewall bot_filter rule";
    ...
    const botShareNote = botShare?.botPct != null ? `bot_fdt_pct=${(botShare.botPct * 100).toFixed(0)}%` : "bot_fdt_pct=unknown";
