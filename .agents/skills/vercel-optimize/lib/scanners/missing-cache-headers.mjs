...
export const metadata = {
    ...
    includeGlobs: ["**/route.{ts,tsx,js,jsx}", "**/api/**/*.{ts,tsx,js,jsx}", "**/page.{ts,tsx,js,jsx}", "**/layout.{ts,tsx,js,jsx}"],
};
...
export function scan({ files }) {
    ...
    for (const { path, content } of files) {
        ...
        const hasGetHandler = /export\s+(async\s+)?function\s+GET/.test(content) || /export\s+const\s+GET\s*=/.test(content);
        if (hasGetHandler) {
            const hasCacheControl = /Cache-Control/i.test(content) || /CDN-Cache-Control/i.test(content) || /export\s+const\s+revalidate\s*=/.test(content);
