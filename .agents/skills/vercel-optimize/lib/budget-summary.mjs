...
export function buildBudgetSummary(gate) {
    ...
    const currentBudget = typeof gate?.budget?.maxCandidates === "number" ? gate.budget.maxCandidates : gate?.budget?.maxCandidates === "all" ? Infinity : 6;

    // Only budget skips can be reached by raising the budget; disqualified/coveredBy can't.
    const skippedByBudget = gated.filter((g) => typeof g.gatedReason === "string" && g.gatedReason.startsWith("skippedByBudget"));
    ...
    const reason = shouldAsk ? `default budget skipped ${skipped} candidate(s); ask user whether to expand` : reasonParts.join("; ") || "no expansion possible";
    ...
    const investigatingPreviewCount =
        typeof currentBudget === "number" && currentBudget <= MAX_FULL_INVESTIGATING_PREVIEW ? currentBudget : TOP_INVESTIGATING_PREVIEW;
