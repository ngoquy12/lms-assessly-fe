...
export function recommendationKey(rec) {
    ...
    const bucket = intent === "cache-control:s-maxage" ? NO_VALUE : String(rec?.bucket ?? NO_VALUE);
    return JSON.stringify([bucket, dedupEditTarget(rec), primarySkillRule(rec), intent]);
}
...
export function fixShape(rec) {
    ...
    const primaryText = [rec?.fix, rec?.desiredBehavior].filter((v) => typeof v === "string" && v.trim()).join("\n");
...
function firstAffectedFile(rec) {
    ...
    const referenced = referencedCodeFiles(rec).find((file) => direct.includes(file));
    if (referenced) return referenced;
    return Array.isArray(rec?.affectedFiles) ? rec.affectedFiles[0] : null;
}

function affectedFiles(rec) {
    return Array.isArray(rec?.affectedFiles) ? rec.affectedFiles.map(normalizePath).filter((file) => file !== NO_VALUE) : [];
}
...
function referencedCodeFiles(rec, fields = ["what", "why", "fix", "currentBehavior", "desiredBehavior", "verify"]) {
    ...
    return unique(matches.map((m) => normalizePath(m.replace(/^[\s`'"(]+/, ""))).filter((file) => file !== NO_VALUE));
}

function isSMaxageCacheHeaderRec(rec) {
    const text = [rec?.what, rec?.why, rec?.fix, rec?.desiredBehavior, ...(Array.isArray(rec?.citations) ? rec.citations : [])].filter(Boolean).join("\n");
    return /\bs-maxage\b/i.test(text) && /\b(?:Cache-Control|CDN cache|cdn-cache|caching\/cdn-cache)\b/i.test(text);
}

function isCacheLifeRec(rec) {
    const text = [rec?.candidateRef, rec?.what, rec?.why, rec?.fix, rec?.desiredBehavior, ...(Array.isArray(rec?.citations) ? rec.citations : [])]
        .filter(Boolean)
        .join("\n");
    return /^isr_overrevalidation:/.test(String(rec?.candidateRef ?? "")) && /\bcacheLife\s*\(|\bcacheLife\b/i.test(text);
}
...
function sharedFunctionTarget(rec) {
    ...
    const text = [rec?.what, rec?.why, rec?.fix, rec?.currentBehavior, rec?.desiredBehavior].filter((v) => typeof v === "string" && v.trim()).join("\n");
    const names = [...text.matchAll(/\b(?:get|fetch|load|read|render|create|generate|filter|resolve)[A-Z][A-Za-z0-9_]*\b/g)].map((m) => m[0]);
    const stop = new Set(["getPayload", "draftMode", "notFound", "redirect", "Promise", "Response", "NextResponse"]);
    ...
    return [...score.entries()].sort((a, b) => b[1] - a[1] || text.indexOf(a[0]) - text.indexOf(b[0])).map(([name]) => `function:${name}`)[0] ?? null;
}

function cacheLifeIntent(rec) {
    const text = [rec?.what, rec?.why, rec?.fix, rec?.desiredBehavior, rec?.verify].filter(Boolean).join("\n");
    const profiles = unique([...text.matchAll(/\bcacheLife\s*\(\s*['"`]([^'"`]+)['"`]/g)].map((m) => m[1]));
    const tags = unique([
    ...
    return ["next-cache:cache-life", profiles.join("|") || NO_VALUE, tags.join("|") || NO_VALUE, invalidation].join(":");
}
...
function withDedupMetadata(rec) {
    ...
    const count = Math.max(numericCount(rec.corroborationCount), 1 + existing.length);
    return existing.length > 0 || count > 1 ? { ...rec, appliesAlsoTo: existing, corroborationCount: count } : { ...rec };
}
...
function mergeDuplicateRecs(a, b) {
    ...
    const appliesAlsoTo = uniqueAppliesAlsoTo([...winnerExisting, appliesAlsoEntry(loser), ...loserExisting]);
    const corroborationCount = numericCount(winner.corroborationCount) + numericCount(loser.corroborationCount);
    return {
        ...winner,
        appliesAlsoTo,
...
function signalMagnitude(rec) {
    const text = [rec?.o11ySignal, rec?.why, rec?.what, rec?.impact].filter((v) => typeof v === "string" && v.trim()).join("\n");
...
function appliesAlsoEntry(rec) {
    return {
        ...
        affectedFiles: Array.isArray(rec?.affectedFiles) ? rec.affectedFiles.map(normalizePath).filter((p) => p !== NO_VALUE) : [],
...
function normalizedAppliesAlsoTo(entries) {
    ...
    return entries
        ...
        .map((e) => ({
            ...
            affectedFiles: Array.isArray(e.affectedFiles) ? e.affectedFiles.map(normalizePath).filter((p) => p !== NO_VALUE) : [],
...
function uniqueAppliesAlsoTo(entries) {
    ...
    for (const entry of entries) {
        const key = JSON.stringify([entry.candidateRef ?? NO_VALUE, entry.affectedFiles?.join(",") ?? NO_VALUE, entry.what ?? NO_VALUE]);
