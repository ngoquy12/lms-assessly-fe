...
export async function libraryForStack(framework, version) {
    const lib = await loadLibrary();
    const matches = (frameworks) => frameworks.some((p) => matchesFrameworkVersion(p, framework, version) || p === "*");
    return {
...
export async function sanitizeCitations(rec, framework, version) {
    ...
    for (const cite of rec.citations ?? []) {
        ...
        if (entry.applicableFrameworks.includes("*") || entry.applicableFrameworks.some((p) => matchesFrameworkVersion(p, framework, version))) {
            kept.push(cite);
        } else {
            strippedVersion.push(cite);
        }
