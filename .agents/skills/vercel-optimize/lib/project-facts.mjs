...
export function deriveProjectFacts(signals) {
    ...
    if (cfg.functionDefaultMemoryType === "standard") {
        ...
        out.push({
            ...
            contradictPhrases: ["upgrade memory to performance", "upgrade to performance memory", "switch to performance memory", "enable performance memory"],
        });
    }
    ...
    if (cfg.functionZeroConfigFailover === true) {
        out.push({
            ...
            contradictPhrases: ["enable zero-config failover", "enable multi-region failover", "turn on zero-config failover"],
        });
    }
    return out;
}
...
export function findRecContradictions(rec, facts) {
    ...
    const haystack = [rec.what, rec.fix, rec.desiredBehavior, rec.currentBehavior].map((s) => (typeof s === "string" ? s.toLowerCase() : "")).join("\n");
    if (!haystack) return [];
    return facts.filter((f) => (f.contradictPhrases ?? []).some((p) => haystack.includes(p.toLowerCase())));
}
