...
function renderFinalRecommendationPreview(recommendations, signals, maxRecommendations) {
    const ready = Array.isArray(recommendations) ? sortRecs(recommendations.filter((r) => r && r.abstain !== true)) : [];
...
function renderCoverageLine(candidates, recommendations, signals, opts = {}) {
    ...
    const skippedByBudget = candidates.filter((c) => typeof c.gatedReason === "string" && c.gatedReason.startsWith("skippedByBudget"));
    const coveredByDedup = candidates.filter((c) => typeof c.gatedReason === "string" && c.gatedReason.startsWith("coveredBy"));
    const disqualified = candidates.filter((c) => typeof c.gatedReason === "string" && c.gatedReason === c.disqualifyReason);
    ...
    const rawNoChangeCount = Number.isInteger(opts.noChangeCount) ? opts.noChangeCount : Array.isArray(opts.abstentions) ? opts.abstentions.length : 0;
...
function renderMetadataLine(stack, plan, usage, signals = {}) {
    ...
    const period = usage?.period ? `${usage.period.from ?? "?"} → ${usage.period.to ?? "?"}` : "(unavailable)";
    ...
    const planLabel = plan.plan === "uncertain" ? `${plan.plan} (${plan.reason ?? "no signal"})` : (plan.plan ?? "unknown");
...
function renderStrengths(signals) {
    ...
    if (totalInv > 1000) {
        ...
        if (coldPct < 0.02)
            lines.push(`- Cold-start rate is very low (${(coldPct * 100).toFixed(2)}%) — Fluid Compute or warm-instance reuse is doing its job.`);
    }
...
function renderConfigurationNotes(signals) {
    const projectFacts = deriveProjectFacts(signals);
    return projectFacts.filter((f) => String(f.id ?? "").startsWith("memory_")).map((f) => `- ${f.strength}`);
}
...
function renderDataGaps(signals) {
    ...
    if (cwvMetric.failed) {
        ...
        if (cwv === 0)
            lines.push("- No Speed Insights measurements — Core Web Vitals analysis dormant. Wire up Speed Insights to enable LCP/INP/CLS recommendations.");
    }
    ...
    if (middlewareMetric.collected) {
        ...
        if (middleware.length === 0)
            lines.push("- No middleware invocations — either no `middleware.ts` is shipped or its matcher excludes all observed traffic.");
    }
