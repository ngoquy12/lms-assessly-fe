...
export function gradeRecommendation(rec, ctx = {}) {
    ...
    const overall = roundTo(grounding * W.grounding + evidence * W.evidence + specificity * W.specificity + actionability * W.actionability, 4);
    return {
        specificity,
        actionability,
        grounding,
...
function scoreEvidence(rec) {
    const text = [rec.what, rec.why, rec.fix, rec.verify].filter((x) => typeof x === "string").join("\n");
    if (!text) return 0;
    ...
    let s = Math.min(0.5, filelines * 0.2) + Math.min(0.3, units * 0.075) + Math.min(0.2, counts * 0.05);
    return Math.min(1, roundTo(s, 4));
}
