...
function recommendsSharedCache(rec) {
    const haystack = [rec?.what, rec?.why, rec?.fix, rec?.desiredBehavior, rec?.verify].filter(Boolean).join("\n");
...
function mentionsNextCachedNotFound(rec) {
    ...
    return /\bnotFound\b/.test(haystack) && /['"`]use cache['"`]|\buse cache\b/i.test(haystack) && /\b(?:500|5xx|error rate|errors?)\b/i.test(haystack);
}

function mentionsNextStableCacheApi(rec) {
    const haystack = recText(rec);
    return /\bunstable_(?:cacheLife|cacheTag)\b/.test(haystack) || /\brevalidateTag\s*\([^)]*['"`][^'"`]+['"`]\s*\)/.test(haystack);
}

function mentionsNext16RuntimeCacheApiMismatch(rec) {
    const haystack = recText(rec);
    const citations = asArray(rec?.citations).join("\n");
    return /\bunstable_cache\b/.test(haystack) && (/\bRuntime Cache\b/i.test(haystack) || /vercel\.com\/docs\/caching\/runtime-cache/i.test(citations));
}
...
function mentionsRuntimeCacheWhenCacheComponents(rec) {
    ...
    const citations = asArray(rec?.citations).join("\n");
    return (
        /\b(?:Runtime Cache|@vercel\/functions|getCache\s*\(|setCache\s*\()\b/i.test(haystack) || /vercel\.com\/docs\/caching\/runtime-cache/i.test(citations)
    );
}
...
function mentionsNextImagePriorityRecommendation(rec) {
    ...
    return /\b(?:set|add|use|enable|mark|make|turn on|with)\b[^.\n]{0,120}\bpriority\b/i.test(haystack) || /<Image\b[^>]*\bpriority(?:\s|=|>)/i.test(haystack);
}
...
function mentionsCatchToNotFound(rec) {
    if (!/^route_errors:/.test(String(rec?.candidateRef ?? ""))) return false;
    const haystack = recText(rec);
    return /\bcatch\b/i.test(haystack) && /\b(?:404|not[- ]found|not found|notFound)\b/i.test(haystack);
}

function mentionsIgnoredBuildStepRecommendation(rec) {
    const haystack = recText(rec);
    return (
        /\b(?:Ignored Build Step|ignoreCommand|turbo-ignore|skip unaffected|unaffected projects?)\b/i.test(haystack) &&
