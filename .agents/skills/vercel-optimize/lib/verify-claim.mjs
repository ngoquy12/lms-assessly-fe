...
async function verifyCacheVaryMatchesDynamicInputs({ rec, files, repoRoot = ".", projectRootDirectory = null }) {
    ...
    const text = [rec.what, rec.why, rec.fix, rec.currentBehavior, rec.desiredBehavior, rec.verify].filter(Boolean).join("\n");
...
async function verifyNextStableCacheApiForVersion({ rec, framework, frameworkVersion }) {
    ...
    if (/\brevalidateTag\s*\([^)]*['"`][^'"`]+['"`]\s*\)/.test(text) && !/\brevalidateTag\s*\([^)]*['"`][^'"`]+['"`]\s*,/.test(text)) {
        return {
            disposition: "failed",
...
async function verifyNextRuntimeCacheApiForVersion({ rec, framework, frameworkVersion }) {
    ...
    const text = recText(rec);
    const citations = Array.isArray(rec.citations) ? rec.citations.join("\n") : "";
    if (/\bunstable_cache\b/.test(text) && (/\bRuntime Cache\b/i.test(text) || /vercel\.com\/docs\/caching\/runtime-cache/i.test(citations))) {
        return {
            disposition: "failed",
...
async function verifyNextCacheLifetimeFreshnessSupported({ rec, files, repoRoot = ".", projectRootDirectory = null }) {
    ...
    const tags = dedupeCacheTags([...extractCacheTags(text), ...(await extractCacheTagsFromFiles(files, repoRoot, projectRootDirectory))]);
    if (tags.length === 0) {
    ...
    const invalidationFiles = [...recTextAsFile, ...(await readCacheInvalidationFiles(repoRoot, projectRootDirectory))];
...
async function verifyNextCacheComponentsRouteChainFile({ rec, framework, frameworkVersion, cacheComponents, signals }) {
    ...
    const layoutFiles = recommendationFilesFromRec(rec).filter((file) => /(^|\/)layout\.(?:tsx?|jsx?)$/.test(String(file)));
    if (layoutFiles.length === 0) {
        return { disposition: "verified", reason: "no layout files named in recommendation" };
    }
    const layoutRoutes = routeRows.filter(
        (route) => route?.type === "layout" && route?.file && layoutFiles.some((file) => pathSuffixMatches(file, route.file)),
    );
    ...
    const matchingLayout = layoutRoutes.find((layout) => layoutAppliesToCandidateRoute(layout.routePath, target));
    if (matchingLayout) {
        return {
...
async function verifyNextImagePriorityApiForVersion({ rec, framework, frameworkVersion }) {
    ...
    const text = recText(rec);
    if (
        /\b(?:preload|fetchPriority|loading\s*=\s*['"`]eager['"`]|loading:\s*['"`]eager['"`])\b/.test(text) &&
...
async function verifyNextRouteRevalidateStaticPrereq({ rec, framework, cacheComponents, repoRoot = ".", projectRootDirectory = null }) {
    ...
    const files = recommendationFilesFromRec(rec).filter(
        (file) =>
            /(^|\/)app\/.+\/(?:page|layout|template)\.(?:tsx?|jsx?)$/.test(String(file)) || /(^|\/)(?:page|layout|template)\.(?:tsx?|jsx?)$/.test(String(file)),
    );
    if (files.length === 0) {
...
function recText(rec) {
    return [rec?.what, rec?.why, rec?.fix, rec?.currentBehavior, rec?.desiredBehavior, rec?.verify].filter(Boolean).join("\n");
}
...
function layoutAppliesToCandidateRoute(layoutPath, targetRoute) {
    ...
    } else if (layoutTokens.length > 0 && targetTokens.length > 0 && isDynamicPlaceholder(layoutTokens[0]) && layoutTokens[1] === targetTokens[0]) {
        layoutTokens = layoutTokens.slice(1);
    }
...
async function* walkFiles(
    root,
    skip = new Set(["node_modules", ".next", ".vercel", ".turbo", "dist", "build", "coverage", ".git", "content", "fixtures", "migrations", "public"]),
) {
    let entries;
    try {
