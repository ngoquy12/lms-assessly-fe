...
function normalizeTopic(topic) {
    const maxBriefChars = Number.isFinite(topic.maxBriefChars) ? topic.maxBriefChars : DEFAULT_MAX_BRIEF_CHARS;
    return {
        id: topic.id,
        title: topic.title,
        status: topic.status,
...
async function validateSupportTopic(topic) {
    ...
    for (const heading of ["## Investigation Brief", "## Evidence To Check", "## Do Not Recommend When", "## Verification"]) {
...
function matchesFrameworks(frameworks, framework, version) {
    return frameworks.some((pattern) => pattern === "*" || matchesFrameworkVersion(pattern, framework, version));
}
...
function matchesRouter(routers, stack) {
    ...
    return (routers.includes("app") && stack?.hasAppRouter) || (routers.includes("pages") && stack?.hasPagesRouter);
}
...
function matchesCandidateMetrics(metrics, candidate) {
    ...
    const observed = new Set(
        [candidate?.evidence?.metric, ...(candidate?.evidence?.issues ?? []).map((i) => i?.metric)].filter(Boolean).map((m) => String(m).toUpperCase()),
    );
...
function matchesScannerPatterns(patterns, candidate) {
    if (!Array.isArray(patterns) || patterns.length === 0) return true;
    const observed = new Set([...(candidate?.evidence?.patterns ?? []), ...(candidate?.evidence?.deepDive?.patterns ?? [])].filter(Boolean));
...
function topicCitationsApply(topic, candidateKind, framework, version) {
    if (!candidateKind) return false;
    return topic.citations.every((citation) => citationApplies(citation, candidateKind, framework, version));
}
...
async function citationApplies(citation, candidateKind, framework, version) {
    ...
    if (rule) {
        return rule.applicableFrameworks.includes("*") || rule.applicableFrameworks.some((p) => matchesFrameworkVersion(p, framework, version));
    }

    const url = lib.urls.find((u) => u.url === citation);
    if (!url) return false;
    const kindOk = !Array.isArray(url.appliesTo) || url.appliesTo.length === 0 || url.appliesTo.includes(candidateKind);
    const versionOk = url.applicableFrameworks.includes("*") || url.applicableFrameworks.some((p) => matchesFrameworkVersion(p, framework, version));
