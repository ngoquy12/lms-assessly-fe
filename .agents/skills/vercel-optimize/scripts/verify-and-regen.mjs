...
import { applyQualityFloor, gradeRecommendation } from "../lib/grade-recommendation.mjs";
...
async function main() {
    ...
    for (const g of recsGraded) {
        ...
        const contradictions = claimsWithResults.filter((r) => r.disposition === "failed" && r.claim?.type === "does_not_contradict_project_config");
        ...
        const cacheSafetyFailures = claimsWithResults.filter(
            (r) => r.disposition === "failed" && (r.claim?.type === "cache_vary_matches_dynamic_inputs" || r.claim?.type === "cache_vary_cardinality_safe"),
        );
    ...
    const qualityDroppedIndexes = new Set(qualityCheck.dropped.map((d) => recsGraded.findIndex((g) => g.rec === d.rec)).filter((i) => i >= 0));
    const needsReviewIndexes = new Set(recsGraded.filter((g) => g.rec.needsReview === true).map((g) => g.index));
    ...
    const summary = {
        ...
        averagePassRate: recsGraded.length > 0 ? round4(recsGraded.reduce((s, g) => s + g.verification.passRate, 0) / recsGraded.length) : null,
        averageQuality: recsGraded.length > 0 ? round4(recsGraded.reduce((s, g) => s + g.quality.overall, 0) / recsGraded.length) : null,
    };
