...
async function main() {
    ...
    if (!project) {
        throw new Error("NO_PROJECT_ID: pass one as argv, set VERCEL_PROJECT_ID, or run `vercel link` in this directory.");
    }
    ...
    const [accountPlan, contract, usageResult] = await Promise.all([getAccountPlan(project.orgId || scope), getContract(scope), getUsage({ days: 14, scope })]);
    ...
    const output = {
        ...
        usageScope,
        usageTeamTotal,
        usageError: usageResult?.ok ? (usageContextMismatch ? "USAGE_CONTEXT_MISMATCH" : null) : (usageResult?.code ?? "UNKNOWN"),
        stack,
        metrics,
        metricsSchema: schema,
    };
    ...
    if (successes.length === 0) {
        ...
        if (/payment_required/.test(topCode)) {
            const text = failures
                ...
                .join("\n")
                .toLowerCase();
            if (/subscription to observability plus[\s\S]{0,160}required/.test(text) || /observability plus[\s\S]{0,160}not enabled/.test(text)) {
                return {
                    usable: false,
                    blocker: "no_oplus_probe",
