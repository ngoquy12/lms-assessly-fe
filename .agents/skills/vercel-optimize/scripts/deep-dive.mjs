...
import { SCANNER_KINDS, TIME_WINDOW, mergeIntoEvidence, specsForCandidate } from "../lib/deep-dive.mjs";
...
async function main() {
    ...
    const [merged, gate] = await Promise.all([readFile(mergedPath, "utf-8").then(JSON.parse), readFile(gatePath, "utf-8").then(JSON.parse)]);
    ...
    const allCandidates = [...toLaunch.map((c, i) => ({ c, group: "toLaunch", i })), ...platform.map((c, i) => ({ c, group: "platform", i }))];
