...
import { buildResolver, detectMonorepoRoot, listWorkspacePackages, resolveWorkspaceImports } from "../lib/workspace-resolver.mjs";
...
async function main() {
    ...
    const [stack, files, routes] = await Promise.all([detectStack(rootDir), collectFiles(rootDir), enumerateRoutes(rootDir)]);
    ...
    findings.sort((a, b) => a.file.localeCompare(b.file) || (a.line ?? 0) - (b.line ?? 0) || a.pattern.localeCompare(b.pattern));
...
async function enrichRoutesWithWorkspaceImports(routes, scanRootDir, resolver, monorepoRoot) {
    ...
    for (const r of routes) {
        ...
        r.workspaceImports = resolved.slice(0, WORKSPACE_IMPORT_LIMIT_PER_ROUTE).map((abs) => relative(monorepoRoot, abs));
    }
}
...
async function enumerateRoutes(root) {
    ...
    return routes.sort((a, b) => a.routePath.localeCompare(b.routePath) || routeTypeOrder(a.type) - routeTypeOrder(b.type) || a.file.localeCompare(b.file));
}
