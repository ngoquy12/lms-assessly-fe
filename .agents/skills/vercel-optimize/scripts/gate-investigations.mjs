...
import { DEFAULT_MAX_CODE_CANDIDATES, GATE_VERSION, gates } from "../lib/gates/index.mjs";
...
async function main() {
    ...
    const gated = [
        ...gateable.filter((c) => c.disqualified).map((c) => ({ ...c, gatedReason: c.disqualifyReason ?? "disqualified" })),
