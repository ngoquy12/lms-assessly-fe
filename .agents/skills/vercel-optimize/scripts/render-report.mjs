...
const HARD_REGEN_TRIGGERS = new Set(["project_config_contradiction", "cache_vary_safety", "semantic_safety"]);
...
async function main() {
    ...
    const activeCandidates = [...(Array.isArray(gateRaw.toLaunch) ? gateRaw.toLaunch : []), ...(Array.isArray(gateRaw.platform) ? gateRaw.platform : [])];
    const enforceCurrentGate = !Array.isArray(recsRaw) && activeCandidates.length > 0;
    const staleRecommendationDrops = [];
    const wrapperRecommendations = Array.isArray(recsRaw.renderableRecommendations) ? recsRaw.renderableRecommendations : (recsRaw.recsGraded ?? []);
    const needsReviewDrops = [];
    const candidateRecommendations = Array.isArray(recsRaw)
        ? recsRaw.filter((r) => r?.abstain !== true)
        : wrapperRecommendations.filter((r, i) => (r.quality?.overall ?? 0) >= 0.55).filter((r) => !hardRegenRefs.has(r.candidateRef));
    ...
    const readyTargets = new Set(recommendations.map((r) => candidateTarget(r?.candidateRef)).filter(Boolean));
