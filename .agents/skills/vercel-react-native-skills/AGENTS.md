...
function ProductList() {
    ...
    return <LegendList data={products} renderItem={({ item }) => <ProductRow name={item.name} price={item.price} imageUrl={item.image} />} />;
}
```
...
function AnimatedButton({ onPress }: { onPress: () => void }) {
    ...
    return (
        <Pressable onPress={onPress} onPressIn={() => (scale.value = withTiming(0.95))} onPressOut={() => (scale.value = withTiming(1))}>
...
```tsx
<Image source={{ uri: url }} placeholder={{ blurhash: "LGF5]+Yk^6#M@-5c,1J5@[or[Q6." }} contentFit="cover" transition={200} style={styles.image} />
```
...
function SettingsMenu() {
    ...
    return (
        <DropdownMenu.Root>
            ...
            <DropdownMenu.Content>
                <DropdownMenu.CheckboxItem key="notifications" value={notifications} onValueChange={() => setNotifications((prev) => !prev)}>
...
function MyScreen() {
    ...
    return (
        <View style={{ flex: 1 }}>
            ...
            <Modal visible={visible} presentationStyle="formSheet" animationType="slide" onRequestClose={() => setVisible(false)}>
                <View>
                    <Text>Sheet content</Text>
                </View>
            </Modal>
