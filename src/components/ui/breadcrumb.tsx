...
const BreadcrumbList = React.forwardRef<HTMLOListElement, React.ComponentPropsWithoutRef<"ol">>(({ className, ...props }, ref) => (
    <ol ref={ref} className={cn("flex flex-wrap items-center gap-1.5 text-sm font-semibold break-words text-slate-500", className)} {...props} />
));
...
>(({ className, ...props }, ref) => {
    return <a ref={ref} className={cn("transition-colors hover:text-brand-600", className)} {...props} />;
});
...
const BreadcrumbSeparator = ({ children, className, ...props }: React.ComponentProps<"li">) => (
    <li role="presentation" aria-hidden="true" className={cn("font-normal text-slate-400 [&>svg]:h-3.5 [&>svg]:w-3.5", className)} {...props}>
