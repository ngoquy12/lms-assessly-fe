...
import { ArrowRight, Brain, Clock, HeartHandshake, HelpCircle, Loader2, RotateCcw, X } from "lucide-react";
...
export function ExamTakingView({ examId }: { examId: string }) {
    ...
    const { data: session, isLoading: isSessionLoading, isError: isSessionError, refetch: refetchSession } = useExamSession();
    const { data: questionsData, isLoading: isQuestionsLoading, isError: isQuestionsError, refetch: refetchQuestions } = useExamQuestions();
    const questions = questionsData ?? [];
    const isLoading = isSessionLoading || isQuestionsLoading;
    const isError = isSessionError || isQuestionsError;

    // Keys for LocalStorage persistence
    const STORAGE_KEY_ANSWERS = `lms_exam_answers_${examId}`;
    const STORAGE_KEY_START_TIME = `lms_exam_start_time_${examId}`;
    const STORAGE_KEY_SAVED_AT = `lms_exam_last_saved_${examId}`;
    ...
    useEffect(() => {
        ...
        try {
            ...
            console.warn("Could not restore exam answers from localStorage:", err);
        } finally {
            setIsHydrated(true);
        }
    ...
    useEffect(() => {
        ...
        try {
            ...
            console.warn("Could not persist exam answers to localStorage:", err);
            setIsSaving(false);
        }
    ...
    const handleConfirmSubmit = useCallback(() => {
        ...
        setTimeout(() => {
            // Clean up localStorage keys upon submission
    ...
    }, [examId, router, STORAGE_KEY_ANSWERS, STORAGE_KEY_START_TIME, STORAGE_KEY_SAVED_AT]);

    // 3. Anti-cheat countdown timer based on persistent start timestamp
    useEffect(() => {
        ...
        return () => clearInterval(interval);
    }, [session, STORAGE_KEY_START_TIME, handleConfirmSubmit]);
    ...
    // Total display cells for matrix palette (36 cells)
                                        ...
                                        {q.type === "ORDERING" && q.orderingItems && (
                                            <div className="space-y-4 pt-2">
                                                    ...
                                                    <div className="flex min-h-[56px] flex-wrap items-center gap-2 rounded-xl border-2 border-dashed border-slate-200 bg-white p-3">
                                                        {(ans?.orderedItems?.length ?? 0) === 0 ? (
                                                            <span className="text-xs font-medium text-slate-400 italic">
                                                                Bấm vào các từ gợi ý bên dưới để thêm vào đây theo đúng thứ tự...
                                                            </span>
                                                        ) : (
                                                ...
                                                <div className="space-y-2">
                                                    <span className="text-xs font-semibold text-slate-500">Các từ / cụm từ cần sắp xếp (bấm vào để chọn):</span>
                                                        ...
                                                        <div>
                                                            ...
                                                            >
                                                                <SelectTrigger className="h-11 w-full rounded-xl border-slate-300 bg-white text-sm">
                                                                    <SelectValue placeholder="-- Chọn định nghĩa phù hợp --" />
                                                                </SelectTrigger>
                                        ...
                                        <Tooltip>
                                            <TooltipTrigger asChild>
                                                <div className="flex cursor-help items-center justify-between rounded-xl border border-emerald-200/80 bg-emerald-50/70 px-2.5 py-1.5 text-xs font-medium text-emerald-800 transition-colors hover:bg-emerald-100/60">
                                                    ...
                                                    {lastSavedTime && (
                                                        <span className="font-mono text-[11px] font-semibold text-emerald-700">{lastSavedTime}</span>
                                                    )}
                                                </div>
                                            </TooltipTrigger>
                                            <TooltipContent
                                            ...
                                            const hasAnswer = matchingQuestion
                                                ? Boolean(
                                                      ...
                                                      (answers[matchingQuestion.id]?.textAnswer &&
                                                          answers[matchingQuestion.id].textAnswer!.trim().length > 0) ||
                ...
                <Dialog open={isSubmitModalOpen} onOpenChange={setIsSubmitModalOpen}>
                    <DialogContent size="md" className="max-w-[460px] gap-4 rounded-xl border border-slate-200 bg-white px-[24px] py-[20px] shadow-xl">
                        ...
                        <div className="space-y-1 rounded-xl border border-slate-100 bg-slate-50 p-3.5 text-left">
                            <p className="line-clamp-1 text-sm font-bold text-slate-900">{session?.title || "Bài khảo thí"}</p>
